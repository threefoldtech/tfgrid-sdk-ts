<template>
  <div class="border px-4 pb-4 rounded position-relative">
    ...
    <v-card class="white--text pa-4">
      <v-row class="text-center pt-4">
        <v-col cols="12">
          <v-text-field model-value="Stellar" label="Selected Chain" variant="outlined" readonly />
        </v-col>
      </v-row>
      <v-row class="max_line justify-space-between pa-sm-4">
        <v-btn class="mb-4 mr-2" color="secondary" @click="navigation">
          Learn How?
        </v-btn>
        <div class="mb-2">
          <v-btn color="secondary" class="mr-2" @click="openWithdrawDialog = true">
            Withdraw
          </v-btn>
          <v-btn variant="elevated" class="" @click="openDepositDialog = true">
            Deposit
          </v-btn>
        </div>
      </v-row>
  ...
  <deposit-dialog
    ...
    :deposit-wallet="depositWallet"
    :qr-code-text="qrCodeText"
    :deposit-fee="depositFee"
    :open-deposit-dialog="openDepositDialog"
    :twin-id="profileManager.profile?.twinId"

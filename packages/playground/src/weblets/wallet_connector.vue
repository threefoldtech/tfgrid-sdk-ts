<template>
  <v-dialog max-width="950px" scrollable v-model="openDialog">
    <template #activator="{ props }">
      <v-btn
        variant="elevated"
        class="h-100"
        :color="!loading && !grantedPermission ? 'error' : undefined"
        :href="!loading && !isInstalled ? 'https://github.com/threefoldtech/tf-wallet-connector-extension' : undefined"
        :target="!loading && !isInstalled ? '_blank' : undefined"
        v-bind="wallet.profile ? props : { onClick }"
      >
        <template #prepend>
          <v-progress-circular indeterminate size="20" width="2" color="primary" v-if="loading" />
          <v-icon icon="mdi-close-circle" v-else-if="!isInstalled || (isInstalled && !grantedPermission)" />
          <v-icon icon="mdi-account" size="x-large" v-else />
        </template>

        <span v-if="checkingInstallation">Checking if extension is installed.</span>
        <div v-else-if="!isInstalled" class="text-left">
          <span>
            Extension is not installed!<br />
            <small class="font-weight-bold">Click for more.</small>
          </span>
        </div>
        <span v-else-if="requestingAccess">Trying to granted access permission.</span>
        <div v-else-if="!grantedPermission" class="text-left">
          <span>Failed to granted access permission!</span> <br />
          <small class="font-weight-bold">Click to try again.</small>
        </div>
        <span v-else-if="connecting">Connecting to your Wallet.</span>
    ...
    <v-card v-if="wallet.profile">
      <v-card-title class="text-center bg-primary"> Connect your TFChain Wallet </v-card-title>

      <v-card-text>
        <v-container>
          <v-alert type="info" variant="tonal">
            Please visit
            <a href="https://manual.grid.tf/playground/wallet_connector.html" target="_blank" class="app-link">
              the manual
            </a>
            get started.
          </v-alert>
          ...
          <v-row class="mt-6">
            <v-col cols="8">
              <PasswordInputWrapper #="{ props }">
                <v-text-field label="Your Hex Seed" :model-value="wallet.profile.mnemonic" readonly v-bind="props" />
              ...
              >
                <template #activator="{ props }">
                  <copy-input-wrapper :data="wallet.profile.ssh" #="{ props: copyInputProps }">
                    <v-textarea
              ...
              <v-container class="mb-4">
                <v-row>
                  ...
                  <v-btn
                    variant="outlined"
                    color="primary"
                    :disabled="ssh === wallet.profile.ssh || !ssh"
                    @click="updateSSH"
                    :loading="updatingSSH"
                  >
              ...
              <CopyInputWrapper :data="wallet.profile.twinId.toString()" #="{ props }">
                <v-text-field label="Twin ID" :model-value="wallet.profile.twinId" v-bind="props" />
              </CopyInputWrapper>

              <CopyInputWrapper :data="wallet.profile.address" #="{ props }">
                <v-text-field label="Address" :model-value="wallet.profile.address" v-bind="props" />
              </CopyInputWrapper>
            </v-col>
            ...
            <v-col cols="4" class="text-center">
              <p class="mb-4">
                ...
                <a href="https://manual.grid.tf/getstarted/TF_Connect/TF_Connect.html" target="_blank" class="app-link">
                  ThreeFold Connect
                </a>
                to fund your account
              </p>
              <qrcode-generator
                :data="'TFT:' + bridge + '?message=twin_' + wallet.profile.twinId + '&sender=me&amount=100'"
              />
      ...
      <v-card-actions>
        ...
        <v-btn variant="outlined" @click="openDialog = false">Close</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  setup() {
    ...
    const grantedPermission = ref(false);
    async function requestAccess() {
      requestingAccess.value = true;
      grantedPermission.value = await ThreefoldWalletConnectorApi.requestAccess();
    ...
    async function login(account: { mnemonic: string } | null) {
      ...
      try {
        const grid = await getGrid(account);
        if (!grid) {
    ...
    const loading = computed(() => checkingInstallation.value || requestingAccess.value || connecting.value);
    const onClick = computed(() => {
      if (!loading.value && isInstalled.value) {
        if (grantedPermission.value) return connectToAccount;
    ...
    /* Manage ssh */
    const ssh = ref("");
    const sshHint = clearableRef("", 3_000);
    ...
    async function updateSSH() {
      ...
      try {
        if (!wallet.profile) {
          throw new Error("Error: No profile was found!");
        }

        const grid = await getGrid(wallet.profile);

        if (!grid) {
          throw new Error("Error: No grid was found!");
        }

        await storeSSH(grid, ssh.value);
        wallet.updateSSH(ssh.value);
        sshHint.value = "SSH key updated successfully.";
      } catch (error) {
        console.log(error);
      } finally {
    ...
    async function generateSSH() {
      ...
      try {
        ...
        wallet.updateSSH(keys.publicKey);
        ssh.value = keys.publicKey;
        downloadAsFile("id_rsa", keys.privateKey);
        sshHint.value = "SSH key generated successfully.";
      } catch (error) {
        console.log(error);
      } finally {
    ...
    const _sshHint = computed(() => {
      if (updatingSSH.value) return "Updating your public ssh key.";
      if (generatingSSH.value) return "Generating a new public ssh key.";
      return sshHint.value;
    });
    ...
    return {
      ...
      loading,
      onClick,
      bridge: window.env.BRIDGE_TFT_ADDRESS,
      apps: [
        {

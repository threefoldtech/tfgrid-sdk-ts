...
function encrypt(message: string) {
  ...
  const encryptedMessage = nacl.box(encodedMessage, nonce, keypair.publicKey, userKeypair.secretKey);
...
function decrypt(message: string) {
  ...
  const decryptedMessage = nacl.box.open(encryptedMessage, nonce, userKeypair.publicKey, keypair.secretKey);

  return utils.encodeUTF8(decryptedMessage as Uint8Array);
}

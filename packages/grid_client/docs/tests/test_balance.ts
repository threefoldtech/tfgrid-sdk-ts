...
async function getClient() {
  const gridClient = new GridClient({
    ...
    mnemonic: "rely regret impact scan inner kiss exit demand ladder achieve amazing endless",
  });
  await gridClient.connect();
  const x = await gridClient.balance.getMyBalance();
  console.log(x);
  console.log(
    await gridClient.balance.transfer({ address: "5Fpj5MhLiC2cQk8uhzch9EdKggZYhShRHesN14V5vXcdLAbP", amount: 1 }),
  );

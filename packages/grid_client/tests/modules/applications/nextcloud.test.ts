...
test("TC2692 - Applications: Deploy Nextcloud", async () => {
  /**********************************************
     Test Suite: Grid3_Client_TS (Automated)
     Test Cases: TC2692 - Applications: Deploy nextcloud
     Scenario:
        - Generate Test Data/nextcloud Config/Gateway Config.
        - Select a Node To Deploy the nextcloud on.
        - Select a Gateway Node To Deploy the gateway on.
        - Deploy the nextcloud solution.
        - Assert that the generated data matches
          the deployment details.
        - Pass the IP of the Created nextcloud to the Gateway
          Config.
        - Deploy the Gateway.
        - Assert that the generated data matches
          the deployment details.
        - Assert that the Gateway points at the IP
          of the created nextcloud.
  ...
  const metadata = "{'deploymentType': 'nextcloud'}";
  const description = "test deploying Nextcloud via ts grid3 client";

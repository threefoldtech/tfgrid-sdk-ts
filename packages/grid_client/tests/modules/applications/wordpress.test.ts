...
beforeAll(async () => {
  ...
  deploymentName = "wp" + generateString(10);
  gridClient.clientOptions.projectName = `wordpress/${deploymentName}`;
  gridClient._connect();
  return gridClient;
});
...
test("TC2700 - Applications: Deploy Wordpress", async () => {
  /**********************************************
     Test Suite: Grid3_Client_TS (Automated)
     Test Cases: TC2700 - Applications: Deploy Wordpress
     Scenario:
        - Generate Test Data/Wordpress Config/Gateway Config.
        - Select a Node To Deploy the Wordpress on.
        - Select a Gateway Node To Deploy the gateway on.
        - Deploy the Wordpress solution.
        - Assert that the generated data matches
          the deployment details.
        - Pass the IP of the Created Wordpress to the Gateway
          Config.
        - Deploy the Gateway.
        - Assert that the generated data matches
          the deployment details.
        - Assert that the Gateway points at the IP
          of the created Wordpress.
  ...
  const metadata = "{'deploymentType': 'wordpress'}";
  const description = "test deploying Wordpress via ts grid3 client";
    ...
    machines: [
      {
        ...
        env: {
          ...
          ADMIN_EMAIL: "admin123@wordpress.com",
        },
      },
    ],
    metadata: metadata,
  ...
  for (let i = 0; i <= 250; i++) {
    ...
    await axios
      ...
      .then(res => {
        ...
        expect(res.data).toContain("TF wordpress solution");
        reachable = true;
      })
      .catch(() => {

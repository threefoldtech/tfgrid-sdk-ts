...
test("TC - Deploy casperlabs", async () => {
  /**********************************************
     Test Suite: Grid3_Client_TS (Automated)
     Test Cases: TC - Deploy casperlabs
     Scenario:
        ...
        - Verify the resources of the casperlabs solution.
    **********************************************/
  ...
  try {
    ...
    await ssh.execCommand("zinit").then(async function (result) {
      log(result.stdout);
      expect(result.stdout).toContain("casperlabs: Running");
    });
  } finally {

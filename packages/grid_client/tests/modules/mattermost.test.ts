...
test("TC - Deploy a mattermost", async () => {
  /**********************************************
     Test Suite: Grid3_Client_TS (Automated)
     Test Cases: TC - Deploy a mattermost
     Scenario:
        ...
        - Verify the resources of the mattermost solution.
    **********************************************/
  ...
  const metadata = "{'deploymentType': 'mattermost'}";
  const description = "test deploying mattermost via ts grid3 client";
  ...
  try {
    ...
    await ssh.execCommand("zinit").then(async function (result) {
      log(result.stdout);
      expect(result.stdout).toContain("mattermost: Running");
    });

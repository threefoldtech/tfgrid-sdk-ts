...
test("TC - Deploy nodepilot", async () => {
  /**********************************************
     Test Suite: Grid3_Client_TS (Automated)
     Test Cases: TC - Deploy nodepilot
     Scenario:
        ...
        - Verify the resources of the nodepilot solution.
    **********************************************/
    ...
    machines: [
      {
        ...
        disks: disks,
        flist: "https://hub.grid.tf/tf-official-apps/nodepilot-latest.flist",
  ...
  try {
    ...
    await ssh.execCommand("zinit").then(async function (result) {
      log(result.stdout);
      expect(result.stdout).toContain("nodepilot: Running");
    });
  } finally {

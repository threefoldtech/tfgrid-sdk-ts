...
test("TC - Deploy wordpress", async () => {
  /**********************************************
     Test Suite: Grid3_Client_TS (Automated)
     Test Cases: TC - Deploy wordpress
     Scenario:
        ...
        - Verify the resources of the wordpress solution.
    **********************************************/
    ...
    machines: [
      {
        ...
        disks: disks,
        flist: "https://hub.grid.tf/tf-official-apps/wordpress-latest.flist",
  ...
  try {
    ...
    await ssh.execCommand("zinit").then(async function (result) {
      log(result.stdout);
      expect(result.stdout).toContain("wordpress: Running");
    });

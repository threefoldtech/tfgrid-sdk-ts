...
async function getDeployment(client, vms, gatewayName) {
  try {
    ...
    log(res);
    log(`You can access GitLab via the browser using: http://${gatewayName}.yourdomain.com`);
    log("================= Getting VM deployment information =================");
  } catch (error) {
...
async function main() {
  ...
  const vms: MachinesModel = {
    ...
    machines: [
      {
        ...
        memory: 1024 * 4,
        rootfs_size: 0,
        flist: "https://hub.grid.tf/tf-official-apps/gitlab-latest.flist",
        entrypoint: "/sbin/zinit init",
        env: {
          SSH_KEY: config.ssh_key,
          GITLAB_HOSTNAME: "gent02.dev.grid.tf",
    ...
    metadata: "",
    description: "GitLab machine/node",
  };

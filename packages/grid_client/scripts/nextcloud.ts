...
async function getDeployment(client, vms) {
  try {
    ...
    log(res);
    log(`You can access NextCloud via the browser using: http://captain.${res[0].env.CAPROVER_ROOT_DOMAIN}`);
    log("================= Getting deployment information =================");
  } catch (error) {
    ...
    machines: [
      {
        ...
        env: {
          ...
          NEXTCLOUD_AIO_LINK: "aio_link",
          GATEWAY: "String(has_gateway)",
          IPV4: "String(public_ip.value)",
        },
      },
    ],
    metadata: "",
    description: "NextCloud deployment machine",
  };

  ...
  <section>
    <v-dialog v-model="show" scrollable width="80%">
      ...
      <v-card>
        ...
        <v-card-subtitle>
          Please visit
          <a href="https://manual.grid.tf/playground/wallet_connector.html" target="_blank">the manual</a> get started.
        </v-card-subtitle>
        <v-divider />
...
export default class TfChainConnector extends Vue {
  ...
  public async connect() {
    ...
    try {
      ...
      localStorage.setItem(
        key,
        JSON.stringify({
          mnemonic: cryptr.encrypt(this.mnemonic),
  ...
  public async login() {
    ...
    try {
      ...
      const cryptr = new Cryptr(md5(this.loginPassword), { pbkdf2Iterations: 10, saltLength: 10 });
      const mnemonic = cryptr.decrypt(wallet.mnemonic);
  ...
  /* SSH */
  public sshState: SSHState = SSHState.None;
  public async generateSSH() {
    this.sshState = SSHState.Generating;
